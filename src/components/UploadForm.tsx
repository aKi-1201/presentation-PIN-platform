...
export const UploadForm = forwardRef<UploadFormHandle, UploadFormProps>(
  ({ onSuccess, isDragging: _isDragging = false }, ref) => {
    ...
    const handleReplaceFile = (event: MouseEvent<HTMLButtonElement>) => {
      event.stopPropagation();
      resetDragState();
      fileInputRef.current?.click();
    };
    ...
    const showDragActive = !uploading && isDragActive;

    const dropzoneClassName = [
      "flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed p-10 text-center transition-all duration-200",
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          >
            <Upload
              className={[
                ...
                showDragActive ? "text-blue-500" : "text-slate-400"
              ].join(" ")}
              aria-hidden="true"
            />
            ...
            <p
              className={[
                ...
                showDragActive ? "text-blue-700" : "text-slate-600"
              ].join(" ")}
            >
              {showDragActive
                ? hasSelectedFile
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            value={retention}
            onChange={(event) =>
              setRetention(event.target.value as RetentionOption)
            }
            disabled={uploading}
          >
